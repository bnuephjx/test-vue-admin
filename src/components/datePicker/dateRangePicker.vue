<template>
  <div class="calendarBox">
      <!-- v-clickoutside="handleAutoClose" -->
    <input type="text" @focus="handleFocus" :value="dateValue">
    <div class="calendar" v-show="pickerVisible">
      ...
      <div class="calendar-footer">
          <div class="calendar-footer-btns">
              <span class="btn-clear" @click="handleClearCalendar">清空</span>
              <span class="btn-submit" @click="handleCalendarSubmit">确定</span>
          </div>
      </div>
    </div>
  </div>
...
import DateView from './picker/date-range-picker.vue';
...
export default {
  ...
  'methods': {
    ...
    'handleClearCalendar':function(){
        this.rangeValue = [];
    },
    'handleCalendarSubmit':function(){
        var length = this.rangeValue.length;
        if(length<2){
            return false
        }
        this.pickerVisible = false;
